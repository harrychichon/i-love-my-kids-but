			...
			<section className="Effectiveness">
				...
				<div className="FilterContainer">
					<select className={selectedType?.name.toLowerCase()} name="types"
						...
						{allTypes?.map((type) => (
							type.name === 'stellar' ? null : type.name === 'unknown' ? null :
								<option key={type.name} className={type.name.toLowerCase()}
								        value={type.name.toUpperCase()}>{type.name.toUpperCase()}</option>
						))}
					</select>
				</div>
				{selectedType ? (
